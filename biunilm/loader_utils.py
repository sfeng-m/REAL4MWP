...
def raw_batch_list_to_batch_tensors(batch):
    batch_tensors = []
...
def batch_list_to_batch_tensors(batch, is_train=True):
    new_batch = []
    if is_train:
        for bt in batch:
            new_batch.extend(bt)
        ...
        new_batch = batch
    oov_tokens = [bt[-1] for bt in new_batch]
    max_oov_len = max([len(oov_tok) for oov_tok in oov_tokens])
    oov_tokens = [bt[-1] + ['[PAD]'] * (max_oov_len-len(bt[-1])) for bt in new_batch]
    if max_oov_len > 0:
        extra_zeros = np.zeros(max_oov_len)
    else:
        extra_zeros = None
    ...
    for x in zip(*new_batch):
        if x[0] is None:
            ...
            batch_tensors.append(torch.tensor(x, dtype=torch.long))
    return batch_tensors, oov_tokens
